...
export class LinkedInEmailJobSource extends EmailAlertSource {
  ...
  parseLinkedInAlert(htmlBody) {
    ...
    while ((match = linkRegex.exec(htmlBody)) !== null) {
      ...
      const remainingHtml = htmlBody.substring(match.index + match[0].length, match.index + match[0].length + 800);
      ...
      const companyName = lines[0] || 'LinkedIn Connection';
      const location = lines[1] || 'Remote';
