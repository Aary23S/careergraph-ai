export function normalizeCompany(company) {
  ...
  cleaned = cleaned.replace(/\b(?:inc|llc|ltd|corp|co|gmbh)\b/gi, '');
...
export function normalizePosition(position) {
  ...
  let cleaned = position.trim().toLowerCase();
  cleaned = cleaned.replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, ' ');
  cleaned = cleaned.replace(/\s+/g, ' ').trim();
  ...
  words = words.map(w => {
    switch (w) {
      ...
      case 'sde': return 'software engineer';
      default: return w;
    }
  });
  cleaned = words.join(' ').replace(/\s+/g, ' ');
  cleaned = cleaned.replace(/\b(?:ii|iii|iv|v)\b$/gi, '');
  cleaned = cleaned.replace(/\s+/g, ' ').trim();
  return cleaned || null;
}
...
export function classifySeniority(position) {
  ...
  if (words.includes('junior') || words.includes('jr') || words.includes('associate') || words.includes('entry') || norm.includes('entry level')) return 'junior';
  if (words.includes('vp') || words.includes('president') || words.includes('cfo') || words.includes('cto') || words.includes('ceo') || words.includes('coo') || words.includes('cmo') || words.includes('executive') || words.includes('chief') || norm.includes('vice president')) return 'executive';
  if (words.includes('founder') || words.includes('co-founder') || words.includes('creator') || words.includes('partner')) return 'founder';
  if (words.includes('director') || words.includes('dir') || words.includes('head')) return 'director';
  if (words.includes('manager') || words.includes('mgr')) return 'manager';
  if (words.includes('lead') || words.includes('principal') || words.includes('staff')) return 'lead';
...
export function classifyRoleCategory(position) {
  ...
  if (words.includes('ceo') || words.includes('cto') || words.includes('cfo') || words.includes('coo') || words.includes('vp') || words.includes('president') || words.includes('executive') || words.includes('chief')) return 'executive';
